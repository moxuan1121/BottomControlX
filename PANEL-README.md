...
设置页可连续调节手柄高度、垂直位置和面板最小宽度。手柄以内部白条为中心，外层圆角阴影随高度等比例扩展；白条高度保持为外层的 90%。侧边面板会在设定的最小宽度基础上，根据最长动作名称自动拓宽，最大为 280 点。它不接管底部 Home Indicator 手势，也不提供面板内添加按钮。
...
- 用户空间重启按照 Dopamine 的进程启动方式，以 persona 99、UID 0、GID 0 启动自带且已签名授权的 `/basebin/jbctl reboot_userspace`，并检查命令退出状态。刷新图标依赖 roothide 环境中的 `uicache`。
