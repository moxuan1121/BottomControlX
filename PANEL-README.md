...
这个初始版本固定手柄位置、尺寸和透明度，用于先验证交互与稳定性。它不再接管底部 Home Indicator 手势，也不提供面板内添加按钮。
...
- 用户空间重启使用 Dopamine roothide 的 `libjailbreak` 与 `jbctl reboot_userspace` 路径。刷新图标依赖 roothide 环境中的 `uicache`。
