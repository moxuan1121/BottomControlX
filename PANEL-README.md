...
在 BottomControlX 设置中进入「选择与排序动作」，可从系统与越狱动作、快捷指令、应用快捷方式三类添加项目，调整顺序或移除，并选择小／中／大三档图标。桌面和应用内共用一套全局底部上滑手势。选一项时上滑直接运行，选两项或更多时面板跟随手指拉出，向下划回可以取消。没有选择项目、锁屏或横屏时，保留系统手势。
